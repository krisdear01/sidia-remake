...
import { Microscope, ArrowLeft, Search, AlertCircle, RefreshCw } from 'lucide-react';
import { siauApi, SiauApiError } from '../api/client';
import type { SiauRoom } from '../types';
import { LAB_JENIS_RUANGAN } from '../constants';
import { RoomCard } from '../components/RoomCard';

const matchesLab = (room: SiauRoom): boolean => {
    const nama = room.jenis_ruangan?.nama?.toLowerCase() ?? '';
    return LAB_JENIS_RUANGAN.some((tok) => nama.includes(tok));
};
...
export const LaboratoriumPage: React.FC = () => {
    ...
    const loadFirstPage = async () => {
        ...
        try {
            ...
            setRooms(res.data.filter(matchesLab));
            setNextCursor(res.meta.pagination?.next_cursor ?? null);
        } catch (e) {
            setError(e instanceof SiauApiError ? e : new SiauApiError('UNKNOWN', 0, 'Gagal memuat data laboratorium.'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadMore = async () => {
        ...
        try {
            ...
            setRooms((prev) => [...prev, ...res.data.filter(matchesLab)]);
            setNextCursor(res.meta.pagination?.next_cursor ?? null);
        } catch (e) {
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Laboratorium di Universitas Udayana</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100 mb-8">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Cari laboratorium, kode, atau gedung..."
                            className="block w-full rounded-lg border border-slate-300 bg-slate-50 p-3 pl-10 text-slate-900 placeholder-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-emerald-500 sm:text-sm transition-all"
                            value={searchTerm}
                ...
                {error && (
                    <div className="mb-8 rounded-2xl border border-rose-200 bg-rose-50 p-6 flex items-start gap-4">
                        ...
                        <div className="flex-1">
                            <p className="font-semibold text-rose-900">Tidak dapat memuat data laboratorium</p>
                        ...
                        {nextCursor !== null && !error && (
                            <div className="mt-8 flex justify-center">
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 text-white px-6 py-3 text-sm font-medium hover:bg-emerald-700 disabled:opacity-60"
                                >
